...
<style>
    ...
    .form-control:focus {
        ...
        outline: 0;
        box-shadow: 0 0 10px rgb(0, 165, 255);
    }
</style>
...
<div class="container">
    ...
    <div class="container">
        <div class="container">
            <label for="title">제목</label> <input class="form-control" type="text" id="title" name="title"
                                                 value="${board.title}"/>
        </div>
        <br/>
        <div id="dl_tel" class="container">
            ...
            <div class="contentArea">
                <textarea type="text" id="content" name="content" maxlength="11"
                          class="summernote">${board.content}</textarea>
            </div>
        </div>
        ...
        <c:if test="${principal.user.role ne 'ADMIN'}">
            <div class="container">
                <c:choose>
                    <c:when test="${board.secret == 1}"><span> 비밀글 check →</span> <input type="checkbox"
                                                                                         id="secret-check"
                                                                                         name="moblAgree" class="m-2"
                                                                                         checked="checked"></c:when>
                    <c:otherwise><br/> <span> 비밀글 check →</span> <input type="checkbox" id="secret-check"
                                                                        name="moblAgree" class="m-2"></c:otherwise>
                </c:choose>
                <br/>
            </div>
        </c:if>
