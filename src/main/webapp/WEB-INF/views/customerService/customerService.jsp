...
<section class="notice">
            ...
            <div class="search-window">
                <form action="/" method="get">
                    <div class="search-wrap">
                        <label for="search" class="blind">공지사항 내용 검색</label> <input id="search" type="search" name="q"
    ...
    <div id="board-list">
        <div class="container">
            <table class="board-table">
                ...
                <c:forEach var="notice" items="${notices}">
                    <tr>
                        ...
                        <th><a class="cs-detail" style="color: red;"
                               onclick="location.href='/guest/cs/detail/${notice.id}'"><i
                                class="bi bi-megaphone-fill"></i>&nbsp;${notice.title}</a></th>
                    ...
                    <tr>
                        <c:choose>
                            <c:when test="${board.secret eq 1 }">
                                <td>${board.id}</td>
                                <th><a class="cs-detail" onclick="location.href='/guest/cs/detail/${board.id}'"><i
                                        class="bi bi-lock-fill"></i>&nbsp;${board.title }</a></th>
                            </c:when>
                            <c:otherwise>
                                <td>${board.id}</td>
                        ...
                        <c:forEach var="num" items="${pageNums }">
                            <c:choose>
                                <c:when test="${num eq boards.number}">
                                    <span class=""><a href="/user/cs?page=${num}"
                                                      style="font-weight: bold; font-size: 18px;">${num + 1}</a></span>
                                </c:when>
                                <c:otherwise>
            ...
            <button id="btn-write"
                    class="custom-sm-btn float-right ${principal.username eq null ? isNotlogin : islogin}">글쓰기
            </button>
