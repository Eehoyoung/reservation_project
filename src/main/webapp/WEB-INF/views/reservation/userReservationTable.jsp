                            ...
                            <tr>
                                ...
                                <c:choose>
                                    <c:when test="${reservation.approvalStatus eq 'APPROVED'}">
                                        <td style="text-align: center;"><a type="button" style="border-color: white;"
                                                                           onclick="payForKakao(${reservation.id})"><img
                                                src="/assets/kakao_pay.png"></img></a></td>
                                    </c:when>
                                ...
                                <td style="text-align: center;"><c:if
                                    ...
                                    <c:choose>
                                        <c:when test="${principal.user.reportCount > 2}">
                                            <button onclick="alert('회원님은 신고횟수가 3회 이상으로 리뷰를 작성하실 수 없습니다.')" class="btn"
                                                    style="">리뷰 쓰러가기
                                            </button>
                                        </c:when>
                                        <c:otherwise>
                                            <button onclick="location.href='/guest/review/post_form/${reservation.id}'"
                                                    class="btn" style="">리뷰 쓰러가기
                                            </button>
                                        </c:otherwise>
                                    </c:choose>
                                </c:if></td>
